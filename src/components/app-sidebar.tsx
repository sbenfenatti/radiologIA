...
export default function AppSidebar() {
  ...
  const user = {
    ...
    email: 'dr.silva@radiologia.com',
    photoURL: 'https://picsum.photos/seed/doc/100/100',
  };
            ...
            <DropdownMenuTrigger asChild>
                <button className="flex w-full items-center gap-3 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-colors hover:bg-sidebar-accent focus-visible:ring-2">
                    <Avatar className="h-9 w-9">
                        <AvatarImage src={user.photoURL} alt={user.displayName} data-ai-hint="doctor portrait" />
                        <AvatarFallback>{user.displayName.charAt(0)}{user.displayName.split(' ')?.[1]?.charAt(0) || 'S'}</AvatarFallback>
                    </Avatar>
